...
import cz.Sicka.LandProtection.Convert.Flag.FlagType;

public class FlagManager {
	public static final Flag HIDDEN = new Flag("Hidden", FlagType.AREA_ONLY, null, "Hidden", false);
    public static final Flag ADMIN = new Flag("Admin", FlagType.PLAYER_ONLY, null, "Admin");
    ...
    public static final Flag MEMBER = new Flag("Member", FlagType.PLAYER_ONLY, null, "Member");
	...
	public static void addFlag(Flag flag) {
		if(flag.getParent() !=  null){
			flag.getParent().addChildrenFlag(flag);
		}
		if(flag.getFlagType() == FlagType.ANY || flag.getFlagType() == FlagType.AREA_ONLY){
			areaFlags.put(flag.getName().toLowerCase(), flag);
		}
        validFlags.put(flag.getName().toLowerCase(), flag);
    }
	
	public static Map<String, Flag> getAllFlags(){
		return validFlags;
	}
